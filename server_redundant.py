...
def generate_music():
    ...
    inputs = []
    for _ in range(4):
        inputs.append(encode(inp, app.chars))
    encoded_input = torch.vstack(inputs).to(app.device)
    #encoded_input = encode(inp, app.chars)
    #context = encoded_input.unsqueeze(0).to(app.device)

    output_indices = app.model.generate(encoded_input, max_new_tokens=int(length))

    best_output = None
    for k in output_indices:
        output_indices_list = k.tolist()
        decoded_output = decode(output_indices_list, app.chars)
        if best_output is None or len(decoded_output.split("KEY")) < 2:
             best_output = decoded_output

    try:
        #decoded_output = decode(output_indices_list, app.chars)
        decoded_output = best_output
